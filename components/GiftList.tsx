...
const GiftList: React.FC<GiftListProps> = ({ gifts, currentUser, onClaim, onUnclaim, onDelete, onLogin }) => {
          ...
          <div className="flex flex-wrap justify-center gap-3 md:gap-4 mb-12">
            {categories.map((cat) => (
              <button
                ...
                className={`px-6 py-2 text-sm font-sans tracking-wide rounded-full transition-all duration-300 border ${selectedCategory === cat ? 'bg-serenity text-white border-serenity shadow-md transform scale-105' : 'bg-white/50 text-fineBlack/60 border-transparent hover:bg-white hover:shadow-sm hover:text-fineBlack'}`}
              >
                {cat}
              </button>
            ))}
          ...
          {giftChunks.map((chunk, chunkIndex) => (
            <ScrollReveal key={chunkIndex} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12">
              {chunk.map((gift) => {
                ...
                return (
                  <div key={gift.id} className={`group relative bg-white/80 backdrop-blur-sm border border-serenityLight rounded-sm overflow-hidden transition-all duration-500 hover:shadow-xl ${isClaimedByMe ? 'border-green-200 ring-1 ring-green-100' : ''} ${isFullyClaimed && !isClaimedByMe ? 'opacity-80' : 'hover:border-serenity/30'}`}>
                    
                    {/* Admin Delete Action */}
                    {isAdmin && (
                      <button 
                        onClick={() => onDelete?.(gift.id)}
                        className="absolute top-3 right-3 z-30 p-2 bg-red-50 text-red-500 hover:bg-red-500 hover:text-white rounded-full transition-all shadow-sm opacity-0 group-hover:opacity-100"
                        title="Excluir Presente da Lista"
                      >
                        <Trash2 size={16} />
                      </button>
                    )}
                    ...
                    <div className="aspect-[4/3] overflow-hidden relative">
                      <img src={gift.image} alt={gift.name} className={`w-full h-full object-cover transition-transform duration-700 ${isFullyClaimed && !isClaimedByMe ? 'grayscale opacity-60' : 'group-hover:scale-105'}`} />
                      {isFullyClaimed && !isClaimedByMe && (
                        <div className="absolute inset-0 bg-white/20 flex items-center justify-center">
                           <span className="px-4 py-2 font-serif italic text-lg shadow-sm border backdrop-blur-sm bg-white/90 text-fineBlack border-serenity/20">Já escolhido</span>
                        </div>
                      )}
                      {isClaimedByMe && (
                        <div className="absolute inset-0 bg-green-500/10 flex items-center justify-center">
                           <span className="px-4 py-2 font-serif italic text-lg shadow-sm border backdrop-blur-sm bg-green-50 text-green-800 border-green-200">Escolhido por você</span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="p-8 text-center space-y-4">
                      ...
                      <p className="font-sans text-sm font-light text-fineBlack/60 leading-relaxed min-h-[3rem]">{gift.description}</p>
                      {isClaimedByMe ? (
                         <div className="pt-4 space-y-3">
                           <p className="text-xs text-green-600 font-sans">Você presenteou este item. <br/>{isAnonymous && <span className="text-fineBlack/40">(Modo Anônimo)</span>}</p>
                           <button onClick={() => onUnclaim(gift.id)} className="inline-flex items-center gap-2 px-6 py-2 border border-red-200 text-red-500 font-sans text-xs tracking-wider hover:bg-red-50 transition-colors rounded-sm uppercase">
                             <Trash2 size={14} /> Desmarcar
                           </button>
                         </div>
                      ) : isFullyClaimed ? (
                        <div className="pt-4 border-t border-serenity/20">
                          <div className="flex items-center justify-center gap-2 text-serenityDark font-medium font-sans text-sm">
                            <Check size={16} /> <span>Já presenteado</span>
                          </div>
                        </div>
                      ) : (
                        <button onClick={() => handleOpenModal(gift.id)} className="mt-4 inline-flex items-center gap-2 px-8 py-3 bg-serenity text-white font-sans text-sm tracking-wider hover:bg-serenityDark transition-colors rounded-sm shadow-sm hover:shadow-md">
      ...
      {selectedGiftId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-serenityDark/20 backdrop-blur-sm animate-fade-in">
          <div className="bg-white w-full max-w-md p-8 rounded-sm shadow-2xl relative animate-slide-up border border-white/50">
            <button onClick={handleCloseModal} className="absolute top-4 right-4 text-fineBlack/40 hover:text-fineBlack transition-colors"><X size={24} /></button>
            <h3 className="font-serif text-3xl text-fineBlack mb-2">Confirmar Presente</h3>
            <p className="font-sans text-sm text-fineBlack/60 mb-6">Que alegria! Confirme seu nome como aparecerá na lista ou escolha a opção anônima.</p>
            <form onSubmit={handleSubmitClaim} className="space-y-6">
              <div>
                <label htmlFor="name" className="block text-xs uppercase tracking-wider text-fineBlack/50 mb-2">Seu Nome</label>
                <input type="text" id="name" value={guestName} onChange={(e) => setGuestName(e.target.value)} className="w-full border-b border-fineBlack/10 py-2 text-fineBlack outline-none focus:border-serenity transition-colors bg-transparent placeholder-fineBlack/20" placeholder="Seu nome" required />
              </div>
              <div className="flex items-center gap-3">
                <div className="relative flex items-center">
                  <input type="checkbox" id="anonymous" checked={isAnonymous} onChange={(e) => setIsAnonymous(e.target.checked)} className="peer h-5 w-5 cursor-pointer appearance-none rounded-sm border border-fineBlack/20 checked:border-serenity checked:bg-serenity transition-all" />
                ...
                <label htmlFor="anonymous" className="text-sm font-sans text-fineBlack/70 cursor-pointer select-none">Esconder meu nome apenas na lista pública</label>
              </div>
              <button type="submit" className="w-full py-4 bg-serenity text-white font-sans text-sm tracking-widest hover:bg-serenityDark transition-colors uppercase rounded-sm shadow-md">Confirmar Escolha</button>
            </form>
          </div>
        </div>
      )}
